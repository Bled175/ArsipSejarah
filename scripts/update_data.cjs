...
if (startIndex !== -1) {
    // Already has ProvinceData interface
    const objStartIndex = startIndex + startStr.length;
    objectStr = content.substring(objStartIndex, content.lastIndexOf(';'));
} else {
    const oldStartStr = 'export const provinceData: Record<string, {nama: string, ibukota: string, sejarah: string, menarik: string[]}> = ';
    const oldStartIndex = content.indexOf(oldStartStr) + oldStartStr.length;
    objectStr = content.substring(oldStartIndex, content.lastIndexOf(';'));
}
